...
# ==========================================
# 0. CRITICAL: PREVENT STDOUT POLLUTION
# ==========================================
# Standard output must be kept strictly clean for the MCP JSON-RPC protocol.
# These environment variables stop HuggingFace/Tokenizers from printing warnings.
...
try:
    ...
    sys.stderr.write(f" Warning: RAG Engine failed to load. Error: {e}\n")
    embedder, index, chunks = None, None, None
...
def get_financial_math(cik: str, metric: str) -> str:
    """
    Skill 1: Deterministic Financial Math.
    Use this tool to get accurate SEC numbers and Year-over-Year (YoY) changes.
    
    Args:
        cik: The 10-digit Central Index Key of the company.
        metric: The specific financial metric (e.g., 'total_revenue', 'gross_profit', 'net_income').
    """
    pattern = os.path.join(OUTPUT_DIR, f"numerical_{cik}_*.json")
    files = glob.glob(pattern)
    if not files:
...
def get_specific_filing_item(cik: str, item_name: str) -> str:
    ...
    Args:
        cik: The 10-digit Central Index Key.
        item_name: The item name (e.g., 'Item 1A: Risk Factors', 'Item 7: MD&A').
    """
    pattern = os.path.join(OUTPUT_DIR, f"narrative_{cik}_*.json")
    files = glob.glob(pattern)
    if not files:
...
# ==========================================
# SKILL 4: MACRO SYNTHESIZER (PROS & CONS)
# ==========================================
@mcp.tool()
def get_company_pros_and_cons(cik: str) -> str:
    """
    Skill 4: Macro Pros and Cons Data Gatherer.
    Retrieves both financial performance data and qualitative risk factors 
    for a company simultaneously to build a comprehensive summary.
    
    Args:
        cik: The 10-digit Central Index Key of the company.
    """
    sys.stderr.write(f" Gathering Pros & Cons dataset for CIK: {cik}\n")
    
    # 1. Gather Financial Math Data
    math_summary = "Financial Data: "
    pattern = os.path.join(OUTPUT_DIR, f"numerical_{cik}_*.json")
    files = glob.glob(pattern)
    ...
    if files:
        ...
        inc_statement = data.get('income_statement', {})
        metrics_to_check = ['total_revenue', 'gross_profit', 'net_income', 'operating_cash_flow']
        found_metrics = []
        
        for m in metrics_to_check:
            if m in inc_statement:
                yoy = inc_statement[m].get('yoy_percent_change', 'N/A')
                curr = inc_statement[m].get('current', {}).get('val', 'N/A')
                if isinstance(yoy, (int, float)):
                    yoy = f"{yoy * 100:.2f}%"
                found_metrics.append(f"{m.replace('_', ' ').title()}: {curr} (YoY: {yoy})")
        
        math_summary += " | ".join(found_metrics) if found_metrics else "No target metrics found."
    else:
        math_summary += "No numerical filing data found locally."

    # 2. Gather Narrative RAG Data
    rag_summary = "Qualitative Risk Context:\n"
    if index is not None and embedder is not None:
        query_embedding = embedder.encode(["What are the main strategic risks, competition, and operational challenges?"])
        distances, indices = index.search(np.array(query_embedding), k=20)
        retrieved_chunks = [chunks[i] for i in indices[0]]
        
        filtered_chunks = [c for c in retrieved_chunks if str(cik) in c][:3]
        if filtered_chunks:
            rag_summary += "\n".join(filtered_chunks)
        else:
            rag_summary += "No specific risk chunks isolated for this CIK."
    else:
        rag_summary += "RAG Engine offline."

    # 3. Combine payloads
    combined_payload = {
        "cik": cik,
        "quantitative_findings": math_summary,
        "qualitative_findings": rag_summary
    }
    
    return json.dumps(combined_payload, indent=2)
